...
//#################uncomment for ex3###########################
    ...
    // std::ofstream out("results30.csv");
    // out << "kappa,theta,sigma,rho,NTPB,time,euler,almost_exact,dt,result\n";
    ...
    for (float rho= -0.9; rho< 0.9; rho += 0.2){
        ...
        free(eulerCPU);
        printf("results found in time %f ms for euler and %f ms for the almost exact methode with dt= %f\n", euler_elapsed, almost_elapsed, dt);
